...
| Manual order confirmation resend | `order.resend_confirmation_email` | `Spree::OrderMailer.confirm_email` from `spree_emails` when consumer transactional emails are enabled | Implemented through `order_confirmation_resend` direct send, but duplicate risk exists if `spree_emails` is also active | Choose one owner per store; `spree_plunk` is a lower-risk first receipt handoff because the action is explicit | This does not replace normal checkout completion confirmation. Preserve `confirmation_delivered` semantics and avoid double sends. |
| Order confirmation on checkout completion | `order.completed` | `Spree::OrderMailer.confirm_email` from `spree_emails`; also may send store owner notification | Tracks `spree.order.completed` as a Plunk event only; no transactional send handoff yet | Keep Spree/Rails first | Preserve `notify_customer: false`, `confirmation_delivered`, and store owner notification behavior before considering handoff. |
| Order cancellation | `order.canceled` | `Spree::OrderMailer.cancel_email` from `spree_emails` | Tracks `spree.order.canceled` as a Plunk event only; no transactional send handoff yet | Keep Spree/Rails first | Respect `notify_customer: false`. |
| Shipment shipped notification | `shipment.shipped` | `Spree::ShipmentMailer.shipped_email` from `spree_emails` when consumer transactional emails are enabled | Tracks `spree.shipment.shipped` as a Plunk event only; no transactional send handoff yet | Keep Spree/Rails first | Use the Rails event `shipment.shipped`; do not invent a storefront-only shipped event. |
| Reimbursement notification | `reimbursement.reimbursed` | `Spree::ReimbursementMailer.reimbursement_email` from `spree_emails` when consumer transactional emails are enabled | Tracks `spree.reimbursement.paid` as a Plunk event only; no transactional send handoff yet | Keep Spree/Rails first | If later moved, align the Plunk event name and the Spree fact source so refund/reimbursement wording is not ambiguous. |
...
## Implemented `spree_plunk` Direct-Send Tranche
