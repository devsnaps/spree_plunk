  ...
  describe 'GET #edit' do
    it 'renders operator-facing guidance for the Plunk configuration form' do
      ...
      aggregate_failures do
        ...
        expect(response.body).to include('Send Order Confirmation Emails')
        expect(response.body).to include('order.completed')
        expect(response.body).to include('confirmation_delivered')
        expect(response.body).to include('order.canceled')
        expect(response.body).to include('shipment.shipped')
        expect(response.body).to include('reimbursement.reimbursed')
        expect(response.body).to include('Send Store Owner Notifications')
        expect(response.body).to include('Send Payment Link Emails')
        expect(response.body).to include('Spree::OrderMailer.payment_link_email')
